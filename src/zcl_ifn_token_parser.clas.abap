CLASS zcl_ifn_token_parser DEFINITION
INHERITING FROM zcl_token_parser
  ...
  PUBLIC SECTION.
    CONSTANTS: c_regex TYPE string VALUE 'ifn\s+(.*)'.
    METHODS parse REDEFINITION.
ENDCLASS.



CLASS zcl_ifn_token_parser IMPLEMENTATION.
  METHOD parse.
    FIND ALL OCCURRENCES OF REGEX c_regex IN token IGNORING CASE SUBMATCHES DATA(condition_name).
    IF sy-subrc EQ 0.
      io_tree->add_node( node = VALUE #( ( zcl_token_types=>c_ifn ) ( condition_name ) ) ).
    ELSE.
